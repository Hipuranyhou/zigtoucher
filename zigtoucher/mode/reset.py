...
logger = logging.getLogger("zigtoucher.mode.reset")


class Reset(modebase.Mode):
    """Mode for resetting Touchlink capable devices to factory default."""

    class _State(enum.Enum):
        """Reset mode FSM states."""

        SEND_SCAN = enum.auto()
        RECV_RESP = enum.auto()
        SEND_IDENT = enum.auto()
        SEND_RESET = enum.auto()
    ...
    def __init__(
        ...
        :param identena: Should identify packet be send?
        :type identena: bool
        :param identdur: Duration of identify packet.
        :type identdur: bool
        ...
        self.__csv = csv
        self.__identena = identena
        self.__identdur = identdur
    ...
    def __print_summary(self) -> None:
        ...
        print(
            ...
            + termcolor.colored("to factory default. \n", attrs=["reverse"])
        )

    def _act(self) -> str:
        """Init CSV writer if needed and reset devices using zbt_b.Builder.
            ...
            while not self.timeout() and not kbd.interrupted:
                ...
                if self.__state == self._State.SEND_SCAN:
                    ...
                    if self.__pkt_skippable(pkt):
                        self.__state = self._State.SEND_SCAN
                        continue  # fastpath
                    self.__response_id = pkt.response_id
                    ...
                    if self.__identena:
                        self.__state = self._State.SEND_IDENT
                    else:
                        self.__state = self._State.SEND_RESET

                elif self.__state == self._State.SEND_IDENT:
                    pkt = self.__builder.identify_request(self.__identdur)
                    time.sleep(0.1)  # this seems to give the target time to get ready
                    self._transceiver.send(pkt)
                    if self.__identdur == 0xFFFF:
                        time.sleep(2)  # based on philips
                    else:
                        time.sleep(self.__identdur)
                    self.__state = self._State.SEND_RESET

                elif self.__state == self._State.SEND_RESET:
                    pkt = self.__builder.reset_request()
                    self._transceiver.send(pkt)
